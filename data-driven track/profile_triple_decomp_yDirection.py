...
For each x in SLICE_X_LIST:
  ...
  3. Group all points by their (rounded) y-coordinate -> each group spans all z.
  4. Average the field values within each y-group -> z-arithmetic average.
  5. Plot the four component profiles (gg_rr, gg_ps, gg_ns, gg_rs) vs y.

No ResampleToImage, no hill-surface detection, no artefacts.

Run with pvpython:
    "D:/software/paraView/bin/pvpython.exe" profile_triple_decomp_yDirection.py
"""
...
output_png  = results_dir / "profile_triple_decomp_yDirection.png"
...
# 5 = 0.01 mm resolution, sufficient for any mesh with cell heights > 0.1 mm.
Y_ROUND = 5
...
# ── field definitions ──────────────────────────────────────────────────────
FIELD_NAMES = ["gg_ps", "gg_ns", "gg_rr", "gg_rs"]
FIELD_STYLE = {
    "gg_ps": dict(color="#2ca02c", lw=1.8, ls="--", label=r"$gg_{ps}$  Pure Shear"),
    "gg_ns": dict(color="#d62728", lw=1.8, ls="-.", label=r"$gg_{ns}$  Normal Strain"),
    "gg_rr": dict(color="#1f77b4", lw=1.8, ls="-",  label=r"$gg_{rr}$  Rigid Rotation"),
    "gg_rs": dict(color="#9467bd", lw=1.8, ls=":",  label=r"$gg_{rs}$  Rotation-Shear"),
}
...
for slice_x in SLICE_X_LIST:
    ...
    # Fetch raw point data — this IS the fluid cross-section, nothing else
    data   = unwrap(Fetch(slc))
    pts    = data.GetPoints()
    n      = data.GetNumberOfPoints()
    pd_slc = data.GetPointData()

    # y-coordinate of every mesh point on this slice
    y_pts = np.array([pts.GetPoint(i)[1] for i in range(n)])

    # Group points by y-level (round to Y_ROUND decimals to merge identical levels)
    y_r = np.round(y_pts, Y_ROUND)
    y_unique, y_inv = np.unique(y_r, return_inverse=True)   # sorted ascending
    ...
    for name in FIELD_NAMES:
        ...
        entry[name] = sums / counts          # z-arithmetic average per y-level
...
fig, axes = plt.subplots(
    ...
    squeeze=False,
    sharey=True,        # shared y-axis: blank space at bottom = hill solid
)
...
# fig.suptitle("Triple-decomposition profiles  (z-averaged)", fontsize=13, y=1.01)
